...
class Products extends MY_Controller {
    ...
    /**
     * Affiche la liste des produits disponibles pour l'utilisateur
     */
	...
	public function create() {
		// Titre de la page
        $page['title'] = 'Création d\'un nouveau produit';
        $page['subtitle'] = 'Formulaire de saisie de création d\'un produit';

        // Récupération des données du formulaire
        $data['trading_name'] = $this->input->post('trading_name');
        $data['firm'] = $this->input->post('firm');
        $data['approval'] = $this->input->post('approval');
		$data['recommended_amount'] = $this->input->post('recommended_amount');
        
		$scripts = array('bootstrap-select-ajax-plugin');
		
        // Règles de validation du formulaire
        $this->form_validation->set_rules('trading_name', 'Nom commercial', 'trim|required|max_length[255]|xss_clean');
        $this->form_validation->set_rules('firm', 'Firm', 'trim|required|max_length[255]|xss_clean');
        $this->form_validation->set_rules('approval', 'Approbation', 'trim|required|max_length[255]|xss_clean');
		$this->form_validation->set_rules('recommended_amount', 'Apport recommandé', 'trim|required|numeric|xss_clean');
        ...
        if ($this->form_validation->run()) {
			...
			if (!$this->Product_model->exist($data['trading_name'])) {
			
				if (!$this->Product_model->create(
					array('trading_name' => $data['trading_name'],
						'firm' => $data['firm'],
						'approval' => $data['approval'],
						'recommended_amount' => $data['recommended_amount'])))
				{
					$data['msg'] = "Erreur lors de la création des données dans la table product";
					$this->view('error', $page['title'], $page['subtitle'], $data);
				} else {
					$data['msg'] = "Le produit a été créé avec succès !";
					$this->view('success', $page['title'], $page['subtitle'], $data);
				}
			}
			else {
				$data['msg'] = "Erreur, un produit portant le même nom existe déjà";
				$this->view('error', $page['title'], $page['subtitle'], $data);
			}
        } else {
            // Affichage du formulaire de création d'equipement  et retour des erreurs
            $this->view('product/new_product', $page['title'], $page['subtitle'],$data, $scripts);
        }		
	}
	
	/**
     * Lien de téléchargement du formulaire d'importation des produits
     */
    public function download_form()
    {
        $this->load->helper('download');
        $file_path = 'download_forms/Import_product.xlsx';//chemin absolu de mon fichier dans serveur
        force_download($file_path, NULL);
    }
	
	
	public function import(){
		$page['title'] = "Importation de produit";
		$page['subtitle'] = "Formulaire d'importation des produits";
		...
		if (!$this->upload->do_upload('product_file')) {
			...
			$this->view('product/import', $page['title'], $page['subtitle'], $data, $scripts);
		}
		else {
			try {
				...
				$nbreDeLignesProduct = $objPHPExcelCopyForCountLineNumber->setActiveSheetIndex(0)->getHighestRow();
				
				//definition d'un tableau contenant les colonnes de base du fichier d'import
                $waiting_main_header = array('trading_name','firm','approval','recommended_amount');
				
				if ($nbreDeLignesProduct > 1) {
							...
							if ($cell_row == 1) { //  si en-tete du fichier
								...
								if (isset($header[$cell_column]) && $data_value) {  // Si la colonne de la donnée fait réference à un champ défini dans le header
									switch ($header[$cell_column]) { //test sur le nom du champ
										case 'trading_name':
											if ($this->Product_model->exist($data_value)) {
												array_push($overallMessageErrors, "trading_name, ligne ".$cell_row.", existe déjà dans la base");
								...
								elseif (isset($header[$cell_column])) {
									switch ($header[$cell_column]) { //test sur le nom du champ
										case 'trading_name': //si trading_name
											array_push($overallMessageErrors, "trading_name, ligne ".$cell_row.", absent");
											array_push($overallLineErrors, $cell_row);
											break;
									}
									$import_data[$cell_row][$header[$cell_column]] = NULL;
					...
					if ( count($overallLineErrors)>0 ) {
						...
						$this->view('product/import_success', $page['title'], $page['subtitle'], $data, $scripts);
					}
					else {
						$import_result_product = $this->Product_model->import($import_data); // Import des données
